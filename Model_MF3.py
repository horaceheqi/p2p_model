...
class MF(object):
    ...
    def get_var(self):
        ...
        # 数据格式：手机号 + 观察月 + 1~169变量 + label
        df.columns = ['info', 'm'] + list(range(1, 170)) + ['label']

        times = time.time()
        print('Process data...')
        # 日期变量[1,2,20,21,48,49],将日期变量转换成天数
        times_var = [1, 2, 20, 21, 48, 49]
        for var in times_var:
            df[var].fillna('201906', inplace=True)
            df[var] = df[var].map(int)
            df[var] = df[var].map(str)
            df[var] = df[var].apply(lambda x: (self.Time - datetime.datetime.strptime(x, '%Y%m')).days)

        # 30/90/180/365——成功(失败)申请/总申请
        newfeature_cols = []
        apply_succeed = list(range(4, 20, 2)) + [23, 25, 31, 37, 43, 53, 57, 66, 70, 79, 83, 92, 96]
        for col in apply_succeed:
            df["pDa_" + str(col)] = df.iloc[:, col] / df.iloc[:, col - 1]  # 分/总
            newfeature_cols.append("pDa_" + str(col))

        # 30/90/180/365——金额/笔数
        all_amount = [26, 32, 38, 44] + list(range(105, 120, 2))
        for col in all_amount:
            df["aTOc_" + str(col)] = df.iloc[:, col] / df.iloc[:, col - 1]  # 总金额/总笔数
            newfeature_cols.append("aTOc_" + str(col))

        # 各卡持卡比例
        card_num = [134, 135, 136, 137]
        for col in card_num:
            df["Ratio_" + str(col)] = df.iloc[:, col] / df.iloc[:, col - 1]  # 部分卡/全部卡
            newfeature_cols.append("Ratio_" + str(col))

        # 新增(90-30、180-90、365-180)
        var_30 = [3, 4, 5, 6, 24, 25, 26, 27, 28, 29] + list(range(52, 65))
        var_90 = [7, 8, 9, 10, 30, 31, 32, 33, 34, 35] + list(range(65, 78))
        var_180 = [11, 12, 13, 14, 36, 37, 38, 39, 40, 41] + list(range(78, 91))
        var_365 = [15, 16, 17, 18, 42, 43, 44, 45, 46, 47] + list(range(91, 104))
        df_columns_1 = [str(b) + '-' + str(a) for a, b in zip(var_30, var_90)]
        df_temp_1 = pd.DataFrame(df[var_90].values - df[var_30].values, columns=df_columns_1)
        newfeature_cols += df_columns_1
        df_columns_2 = [str(b) + '-' + str(a) for a, b in zip(var_90, var_180)]
        df_temp_2 = pd.DataFrame(df[var_180].values - df[var_90].values, columns=df_columns_2)
        newfeature_cols += df_columns_2
        df_columns_3 = [str(b) + '-' + str(a) for a, b in zip(var_180, var_365)]
        df_temp_3 = pd.DataFrame(df[var_365].values - df[var_180].values, columns=df_columns_3)
        newfeature_cols += df_columns_3
        print('Extract features finished...', time.time() - times)

        fin_data = pd.concat([df, df_temp_1, df_temp_2, df_temp_3], axis=1)
        newfeature_cols += list(range(1, 170))
        newfeature_cols = ['info'] + newfeature_cols + ['label']

        fin_data = fin_data[newfeature_cols]
    ...
    def load_data(self):
        ...
        df_t = df[df['label'] == 0]  # 正样本
        df_f = df[df['label'] == 1]  # 负样本
        ...
        train_columns = [x for x in df_.columns if x not in ['info', 'month', 'label']]
    ...
    def start_bin(self, trains_x, trains_y):
        ...
        print('start bin...')
        var_list = [var for var in train_.columns if var not in ['info', 'month', 'label']]
        total_bwi = dict()
        # 标准、逾期、多投变量中离散、连续定义
    ...
    def lr_train(df_trains, df_tests):
        ...
        print('训练集效果...')
        pre_train = model_cls.predict(train_)
        prob_train = np.array(model_cls.predict_proba(train_))
        pre_list_train = list()
        ...
        print('验证集效果...')
        pre_test = model_cls.predict(test_)
        prob_test = np.array(model_cls.predict_proba(test_))
        pre_list_test = list()
