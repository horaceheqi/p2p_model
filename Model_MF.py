...
class MF(object):
    ...
    def get_var(self):
        ...
        df_ = df[df_columns].sort_values(['0', '1'], ascending=[1, 0])
        df_.columns = list(range(0, 215)) + ['label']

        print('Process data...')
        time_features = [1, 157, 158, 159, 160, 164, 165]
        for var in time_features:
            df_[var].fillna('201906', inplace=True)
        ...
        ct_amount_var = [2] + list(range(3, 75, 2)) + list(range(98, 136, 2)) + [143, 145, 146]
        ct_count_var = list(range(4, 76, 2)) + list(range(99, 137, 2)) + [144]
        ct_max_var = list(range(75, 98)) + list(range(136, 143))
        once_var = list(range(148, 170)) + list(range(176, 182)) + list(range(187, 192)) + list(range(201, 211))
        overdue_var = list(range(197, 201)) + list(range(211, 215))

        map_overdue = {np.nan: np.nan, 0: 0, 'M1': 1, 'M2': 2, 'M3': 3, 'M3+': 4}
        df_[195] = df_[195].map(map_overdue)
        df_[204] = df_[204].map(map_overdue)

        ct_var_all = ct_amount_var + ct_count_var + list(range(170, 176)) + list(range(182, 187)) + list(range(192, 197))
        df_group = df_.groupby(df_[0])
    ...
    def load_data2():
        ...
        df_t = train_data[train_data['label'] == 0]
        df_f = df_train[df_train['label'] == 1]
        df_t = df_t.iloc[:2700]
        df_f = df_f.iloc[:2700]
        fin_train = df_t.append(df_f)
        fin_test = pd.read_csv('')
        df_train_x = fin_train.drop(['label'], axis=1)
        df_train_y = fin_train['label']
        df_test_x = fin_test.drop(['label'], axis=1)
        df_test_y = fin_test['label']
        return df_train_x, df_test_x, df_train_y, df_test_y
...
if __name__ == '__main__':
    ...
    demo = MF()
    demo.get_var()
    train_x, test_x, train_y, test_y = demo.load_data1()
